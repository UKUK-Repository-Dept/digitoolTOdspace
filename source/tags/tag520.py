...
def convertTag520(tag520,oai_id,categorize):
    ...
    if len(tag520['a']) > 1:
        ...
        ret['lang'] = catalogue.convertLang[language[0]]
        ret['alternative_abstract'] = tag520['a'][1]
        if len(language) > 1:
            ret['alternative_lang'] = catalogue.convertLang[language[1]]
        return(ret)
    abstract = tag520['a'][0]
    if len(abstract) < 100:
        categorize.categorize_item(oai_id,"520: Too short abstract '{}'".format(abstract))
    ...
    if '8' in tag520.keys():
        ret['lang'] = catalogue.convertLang[tag520['8'][0]]
    if '9' in tag520.keys():
        ret['lang'] = catalogue.convertLang[tag520['9'][0]]
    if 'b' in tag520.keys():
